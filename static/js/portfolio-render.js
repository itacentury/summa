/**
 * Portfolio markup: the summary cards, the depot groups, the position rows and
 * the chart legends.
 *
 * Pure module — every export takes a slice of the `GET /api/portfolio` payload
 * and returns an HTML string. Nothing here touches the DOM, `fetch` or `state`,
 * so each rule below is provable without mounting the view. Imports only from
 * leaf modules (of `truncate.js` it uses the string half, not the DOM half).
 */
