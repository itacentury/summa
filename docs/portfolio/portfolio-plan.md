...
| `/api/portfolio/snapshot/new`   | GET    | prefill: active positions per depot, previous value each (plus `previous_carried`, flagging a previous reading that was itself carried forward), suggested next un-snapshotted week, every date already recorded (`snapshot_dates`)                                                                                                                                 |
| `/api/portfolio/snapshot`       | POST   | `{date, rows:[{position_id, value\|null, deposit\|null}]}`; on a new week `value: null` → carried row (`carried = 1`) and `deposit: null` → 0; on a re-post of an existing week a `null` field preserves what is stored; idempotent per `(position_id, date)` via `INSERT … ON CONFLICT(…) DO UPDATE`; a row whose position was closed before that date is rejected |
...
- A position whose previous reading was itself carried (`previous_carried`) gets a
  `last value carried` marker in its name cell, so the row admits that its placeholder and its
  delta compare against a copy. The marker stays once a real value is typed — the comparison
